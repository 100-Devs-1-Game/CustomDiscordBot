...
class Assets(commands.Cog):
    ...
    async def request(self, ctx: discord.ApplicationContext):
        ...
        if ctx.author.name != game_info["owner"]:
            await ctx.respond(
                "❌ Only the game owner can request assets.", ephemeral=True
            )
            return
    ...
    async def listrequests(self, ctx: discord.ApplicationContext):
        ...
        await ctx.respond(
            "Pick an asset type to list requests:", view=view, ephemeral=True
        )
    ...
    async def listaccepted(self, ctx: discord.ApplicationContext):
        view = AssetSelectView(
            self.ASSET_TYPES, {}, action="list", user=str(ctx.author.name)
        )
        await ctx.respond(
            "Pick an asset type to list requests:", view=view, ephemeral=True
        )
...
class AssetRequestModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        assert ANNOUNCE_CHANNEL_ID > 0, (
            "ANNOUNCE_CHANNEL_ID is not set in environment variables."
        )
...
class AssetTypeSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.action == "request":
            ...
            if self.user:
                rows = Database.get_asset_requests_by_type(
                    asset_type, "Accepted", self.user
                )
            else:
...
class AcceptButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"👷 You accepted the asset request *{self.request['content']}**",
            ephemeral=True,
        )
